...
| **Main output** | `state.db` + `MEMORY.md` + complete `graph.json` (with `source_location`) + `memory/graph-3d.html` + `memory/GRAPH_REPORT.md` | `graph.html` + `GRAPH_REPORT.md` + Obsidian wiki |
| **Data source** | All repository files/docs plus typed decisions, facts, procedures | All project files (code, docs, images) |
| **Extracts from** | Full inventory + Markdown links/sections + JS/TS AST + optional harness LLM enrichment | AST (code) + LLM agents (docs, papers, images) |
...
| **AST extraction** | JS/TS deep symbols plus canonical all-file inventory | Full AST on .py, .ts, .js, .go, .rs files |
| **Community detection** | Yes — deterministic communities over the full graph | Yes — finds latent thematic clusters |
| **Visualization** | 3D HTML view + Markdown/graph exports | Interactive HTML, SVG, Obsidian vault, wiki, GraphML, Neo4j |
| **Token cost** | Fixed ~1300 per projection | Variable — can be high (LLM calls for semantic extraction) |
| **LLM dependency** | Optional; uses the installed harness settings when enabled, deterministic fallback always available | Required for semantic extraction (Claude subagents) |
...
- **Language coverage** — the AST symbol layer covers JavaScript/TypeScript (acorn, function-level `calls`), plus Python/Go/Rust/Java/Ruby/PHP symbol walkers with import/call linking; other files still appear in the complete inventory and document graph.
- **Optional LLM variability** — harness providers can be slow or unavailable; every structural/indexing step has a deterministic fallback.
...
- You work on projects you already know or want a persistent, searchable map of
  the repository plus its operational memory
...
- You need paper-citation graphs or multi-paper claim networks (use graphify for those corpora, then `cm import --graphify`)
...
## 2026 Update: shipped feature gaps closed in 0.7.0

Since v0.7.0, CodeMem closes several gaps that earlier tables flagged as weaknesses:
...
- **Visualization & graph export (was graphify's edge):** `cm gx html|svg|graphml|neo4j`, `cm query` (BFS), `cm gc`, `cm scan --deep` / `--relations`, plus the standalone cognitive 3D view in `docs/visualization/`.
- **Full repository graph (new):** `cm init --deep` / `cm update --memory --deep` now produce a complete file/document/section/link graph, install harness hooks/skills, and write a real-data `memory/graph-3d.html`; the individual scan/entity commands are diagnostic compatibility surfaces.
...
- **Image vision (was graphify's edge):** `cm import` runs a vision stage over images before OCR fallback. A vision-capable harness (codex first, then claude/pi/opencode/gemini/qwen/copilot) reads the image natively — layout, diagram components, chart metrics, visible connections — and every seen label becomes a `vision-element` graph node with `depicts` (image→element, EXTRACTED) and inter-element edges (INFERRED). Labels are validated against OCR tokens so hallucinations are rejected; without a reachable vision harness the deterministic tesseract OCR path keeps working. E2E-verified: a 3-box diagram yields 3 elements + 3 depicts edges, queryable via `cm query`.
- **Source locations (was graphify's edge):** every AST symbol node (js/ts via acorn, py/go/rs/java via regex walkers) carries `source_path` + `source_location` (`L<n>`), stored in SQLite, shipped in the portable `graph.json`, and displayed by `cm query` / `cm gn` as `@ path Ln` — the `src + loc` parity.
- **Narrative graph report (was graphify's edge):** `cm report` prints and persists `memory/GRAPH_REPORT.md` with summary + provenance, god nodes (scaffolding excluded), cross-community surprising connections (trivial `defines`/`contains` excluded), suggested questions with runnable commands, and a token-efficiency section.
- **Honest token efficiency (was graphify's edge):** per-query answer tokens vs naive full-corpus tokens — the same definition as graphify's `benchmark` — reported in `cm report`, recorded in `memory/cost.json` (`reduction_x`), and printed by `init --deep`. On tiny corpora it honestly reports <1x instead of a misleading storage ratio.
...
- **[../design/philosophy.md](../design/philosophy.md)** — local-first philosophy and scope
- **[../benchmarks/memory-benchmark.md](../benchmarks/memory-benchmark.md)** — reproducible benchmark methodology
- **[graphify-vs-codemem-e2e.md](graphify-vs-codemem-e2e.md)** and **[graphiti-vs-codemem-e2e.md](graphiti-vs-codemem-e2e.md)** — latest measured comparisons
