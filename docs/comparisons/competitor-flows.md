# Competitor flows: Graphify and claude-mem

> These diagrams are based on the installed `graphify` CLI and the local
> claude-mem v9.1.1 MCP implementation. They are used for architectural
> comparison. For CodeMem's current flow see the [recall flow](../architecture/recall-flow.md) and [graph service](../architecture/graph-service.md).

## 1. Graphify — codebase knowledge graph (Python + networkx)

The main pipelines are **build** (folder to `graphify-out/graph.json`) and **query** (against an existing graph).
...
## 2. claude-mem — cross-session memory for Claude Code (cloud MCP)

An MCP plugin that persists observations, sessions and prompts with remote semantic search.
Its three-level retrieval workflow is documented by the `mem-search` skill.
