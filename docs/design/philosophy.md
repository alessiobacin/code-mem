...
### The memory core requires no persistent server

The memory core needs no daemon and stores data in project-local SQLite. The optional `cm-graphd` service is a separate loopback-only viewer/chat bridge, automatically managed per user and scoped to registered projects; the watch daemon remains a separate background embedding, capture and consolidation worker.
...
One-time setup:
  ...
  cm init --deep                  # full index + harness integration + 3D graph
...
Daily use:
  ...
  cm update --memory --deep       # repeat the full index after changes
...
- Cognitive lifecycle contracts: episodes, evidence, belief state, candidate gating, working sets and contest/verify auditability.
...
- **[../comparisons/comparison.md](../comparisons/comparison.md)** — detailed comparison with other memory systems
