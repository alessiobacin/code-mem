...
body:not(.logic-mode) .logic-label,body:not(.logic-mode) .logic-verb,body:not(.logic-mode) .flow-label,body:not(.logic-mode) .flow-branch{display:none!important}
...
const IT=/^it/i.test(LOGIC.language||'');
const T=IT?{how:'💡 Come funziona',tech:'🔧 Vista tecnica',title:'Come funziona',journeys:'Percorsi',parts:'Parti',intro:'Ogni zona colorata è una parte del software e dentro ci sono i passi che fa. I rombi gialli sono le domande che si pone: esce una freccia per ogni risposta. Scegli un percorso per seguirlo passo passo, oppure clicca un passo.',stale:'Il codice è cambiato da quando è stata scritta questa mappa: alcuni passi potrebbero essere un po’ vecchi.',steps:'passi',part:'Parte',before:'Arriva da',after:'Poi',inJourneys:'Fa parte di',seeTech:'🔧 Vedi i pezzi tecnici di questa parte',back:'← Tutti i percorsi',kinds:{start:'INIZIO',step:'PASSO',decision:'DOMANDA',store:'DOVE SI CONSERVA',end:'FINE'},stats:(j,n)=>j+' percorsi · '+n+' passi'}:{how:'💡 How it works',tech:'🔧 Technical view',title:'How it works',journeys:'Journeys',parts:'Parts',intro:'Each coloured zone is one part of the software, holding the steps it takes. Yellow diamonds are questions it asks itself: one arrow leaves for each answer. Pick a journey to follow it step by step, or click a step.',stale:'The code changed since this map was written; some steps may be slightly out of date.',steps:'steps',part:'Part',before:'Comes from',after:'Then',inJourneys:'Part of',seeTech:'🔧 See the technical pieces of this part',back:'← All journeys',kinds:{start:'START',step:'STEP',decision:'QUESTION',store:'KEPT IN',end:'END'},stats:(j,n)=>j+' journeys · '+n+' steps'};
...
const viewToggle=document.querySelector('#view-toggle'),statsEl=document.querySelector('#stats'),techStats=statsEl.textContent;
function setMode(on){logicMode=on;document.body.classList.toggle('logic-mode',on);group.visible=!on;logicGroup.visible=on;if(on)techMeshes.push(...meshes.splice(0));else meshes.push(...techMeshes.splice(0));hovered=null;selected=null;active=null;hoverNode=null;searchQuery='';searchMatches=new Set();applyHighlight();logicHighlight();viewToggle.textContent=on?T.tech:T.how;statsEl.textContent=on?T.stats(F.journeys.length,F.nodes.length):techStats;if(on){logicIntro();focusIds(ids)}else{clearSelection();focus={from:camera.position.clone(),to:home.position.clone(),targetFrom:controls.target.clone(),targetTo:home.target.clone(),start:performance.now(),duration:1000}}try{localStorage.setItem('cm-graph-view',on?'logic':'tech')}catch{}}
viewToggle.addEventListener('click',()=>setMode(!logicMode));
