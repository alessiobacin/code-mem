...
function export3DHTML(g, cwd, logic = null, docs = null) {
  ...
  const logicPayload = JSON.stringify(logic && logic.parts?.length
    ? { stale: Boolean(logic.stale), language: logic.language || "", parts: logic.parts, flows: logic.flows || [], flow: logic.flow || null }
    : null).replace(/</g, "\\u003c").replace(/>/g, "\\u003e").replace(/\u2028/g, "\\u2028").replace(/\u2029/g, "\\u2029");
  const docsPayload = JSON.stringify(docs && docs.docs?.length
    ? { topics: docs.topics, docs: docs.docs.map(({ hash, ...doc }) => doc), relations: docs.relations || [] }
    : null).replace(/</g, "\\u003c").replace(/>/g, "\\u003e").replace(/\u2028/g, "\\u2028").replace(/\u2029/g, "\\u2029");
  const hasLogic = Boolean(logic && logic.parts?.length), hasDocs = Boolean(docs && docs.docs?.length);
  const viewButtons = hasLogic || hasDocs
    ? `<div id="views" role="tablist">${hasLogic ? '<button type="button" data-view="logic">💡 How it works</button>' : ""}${hasDocs ? '<button type="button" data-view="docs">📚 Documentation</button>' : ""}<button type="button" data-view="tech">🔧 Technical</button></div>`
    : "";
  const typeColors = {
...
#views{display:flex;gap:4px;flex:none;background:var(--panel);border:1px solid var(--line);border-radius:9px;padding:3px}#views button{border:0;background:transparent;padding:6px 10px;white-space:nowrap;color:#cbd5e1}#views button.on{background:#0f3346;color:#fff;font-weight:600;box-shadow:inset 0 0 0 1px rgba(103,232,249,.55)}
body[data-view=logic] .label,body[data-view=logic] .macro-label,body[data-view=logic] #filters,body[data-view=logic] #search,body[data-view=docs] .label,body[data-view=docs] .macro-label,body[data-view=docs] #filters,body[data-view=docs] #search{display:none!important}
...
body:not([data-view=logic]) .logic-label,body:not([data-view=logic]) .logic-verb,body:not([data-view=logic]) .flow-label,body:not([data-view=logic]) .flow-branch,body:not([data-view=docs]) .doc-label,body:not([data-view=docs]) .topic-label,body:not([data-view=docs]) .doc-rel{display:none!important}
.doc-label{font-size:12px;padding:3px 8px;border-radius:7px;background:rgba(3,14,27,.9);border:1px solid var(--c,#94a3b8);color:#f1f5f9;white-space:nowrap;cursor:pointer;pointer-events:auto;transition:opacity .2s}.topic-label{display:flex;align-items:center;gap:7px;padding:6px 11px;border-radius:999px;background:rgba(3,14,27,.86);border:1px solid var(--c,#67e8f9);color:#f8fafc;font-size:15px;white-space:nowrap;cursor:pointer;pointer-events:auto}.doc-rel{font-size:11px;color:#e2e8f0;background:rgba(3,14,27,.82);padding:1px 6px;border-radius:5px;white-space:nowrap;pointer-events:none;border-left:3px solid var(--c,#94a3b8)}.rel-key{display:flex;align-items:center;gap:7px;padding:3px 0;font-size:12px}.rel-key i{display:inline-block;width:18px;height:3px;border-radius:2px;background:var(--c)}.doc-type{display:inline-block;font-size:10px;text-transform:uppercase;letter-spacing:.06em;color:#94a3b8;border:1px solid var(--line);border-radius:4px;padding:0 4px;margin-left:4px}
.flow-label{display:flex;align-items:center;gap:5px;font-size:12px;padding:3px 8px;border-radius:7px;background:rgba(3,14,27,.9);border:1px solid var(--c,#94a3b8);color:#f1f5f9;white-space:nowrap;cursor:pointer;pointer-events:auto;transition:opacity .2s}.flow-label.decision{border-style:dashed;color:#fde68a}.flow-label.start{font-weight:700}
body.journey-active .logic-label{display:none!important}
.flow-branch{font-size:11px;color:#fde68a;background:rgba(3,14,27,.82);padding:1px 6px;border-radius:5px;white-space:nowrap;pointer-events:none;transition:opacity .2s}
.journey-btn{display:block;width:100%;text-align:left;margin:6px 0;padding:8px 10px;line-height:1.35}.journey-btn small{display:block;color:#94a3b8;margin-top:2px}
.step-row{padding:5px 0;border-bottom:1px solid rgba(148,163,184,.12);cursor:pointer}.step-row:hover{color:#fff}.step-row .branch{display:block;margin-left:22px;color:#fde68a;font-size:12px}.step-num{display:inline-block;min-width:20px;color:#67e8f9}
...
<body>
<div id="scene"></div>
<div id="top"><h1>Code-Mem · 3D Knowledge Graph</h1><span id="stats"></span>${viewButtons}<input id="search" class="search" placeholder="Search nodes…" aria-label="Search nodes"><button id="reset">Reset view</button></div>
...
// View controller: Technical (evidence graph), How it works, Documentation.
const VIEWS={},statsEl=document.querySelector('#stats'),techStats=statsEl.textContent,techStash=[];let currentView='tech';
VIEWS.tech={enter(){group.visible=true;meshes.push(...techStash.splice(0));statsEl.textContent=techStats;clearSelection();focus={from:camera.position.clone(),to:home.position.clone(),targetFrom:controls.target.clone(),targetTo:home.target.clone(),start:performance.now(),duration:1000}},leave(){group.visible=false;techStash.push(...meshes.splice(0));hovered=null;selected=null;searchQuery='';searchMatches=new Set();applyHighlight()}};
function showView(name){if(!VIEWS[name])name='tech';if(name!==currentView)VIEWS[currentView].leave();currentView=name;document.body.dataset.view=name;document.querySelectorAll('#views [data-view]').forEach(b=>b.classList.toggle('on',b.dataset.view===name));VIEWS[name].enter();try{localStorage.setItem('cm-graph-view',name)}catch{}}

if(LOGIC&&!LOGIC.flow){
// "How it works" (parts only, no flowchart yet): plain-language parts as big spheres, flows as arrows.
...
function setMode(on){logicMode=on;logicGroup.visible=on;hovered=null;selected=null;logicActive=null;logicHover=null;searchQuery='';searchMatches=new Set();applyHighlight();logicHighlight();if(on)statsEl.textContent=LOGIC.parts.length+' parts · '+LOGIC.flows.length+' connections';if(on){logicIntro();focusLogic(LOGIC.parts.map(p=>p.id))}}
VIEWS.logic={enter:()=>setMode(true),leave:()=>setMode(false)};
...
if(LOGIC&&LOGIC.flow){
// "How it works" flowchart: parts are coloured zones, journeys are paths of
// start/step/decision/store/end nodes; decisions branch with labelled arrows.
// UI strings are always English; only the map content follows the README language.
const T={how:'💡 How it works',tech:'🔧 Technical view',title:'How it works',journeys:'Journeys',parts:'Parts',intro:'Each coloured zone is one part of the software, holding the steps it takes. Yellow diamonds are questions it asks itself: one arrow leaves for each answer. Pick a journey to follow it step by step, or click a step.',stale:'The code changed since this map was written; some steps may be slightly out of date.',steps:'steps',part:'Part',before:'Comes from',after:'Then',inJourneys:'Part of',seeTech:'🔧 See the technical pieces of this part',back:'← All journeys',kinds:{start:'START',step:'STEP',decision:'QUESTION',store:'KEPT IN',end:'END'},stats:(j,n)=>j+' journeys · '+n+' steps'};
const F=LOGIC.flow;const logicGroup=new THREE.Group();logicGroup.visible=false;scene.add(logicGroup);
const PALETTE=[0x67e8f9,0xa78bfa,0x34d399,0xf472b6,0x60a5fa,0xfdba74,0xa3e635,0xfb7185,0xfbbf24];
const partIndex=new Map(LOGIC.parts.map((p,i)=>[p.id,i])),partById=new Map(LOGIC.parts.map(p=>[p.id,p])),nodeById=new Map(F.nodes.map(n=>[n.id,n]));
const partColor=id=>partIndex.has(id)?PALETTE[partIndex.get(id)%PALETTE.length]:0x94a3b8;
const hex=c=>'#'+c.toString(16).padStart(6,'0');
const KIND_COLOR={start:0x34d399,decision:0xfbbf24,store:0xa78bfa,end:0xfb7185};const KIND_ICON={start:'▶',step:'•',decision:'◆',store:'🗄',end:'■'};
const outLinks=new Map(),inLinks=new Map();F.links.forEach(l=>{if(!outLinks.has(l.from))outLinks.set(l.from,[]);outLinks.get(l.from).push(l);if(!inLinks.has(l.to))inLinks.set(l.to,[]);inLinks.get(l.to).push(l)});
// deterministic force layout: repulsion, springs on links, pull to part centre
const N=F.nodes.length,zoneR=Math.max(34,Math.sqrt(N)*8.5);const centre=new Map(LOGIC.parts.map((p,i)=>[p.id,localPos(i,LOGIC.parts.length,zoneR)]));
const pos=new Map(),vel=new Map(),groups=new Map();F.nodes.forEach(n=>{const k=n.part||'_';if(!groups.has(k))groups.set(k,[]);groups.get(k).push(n)});
groups.forEach((list,k)=>{const c=centre.get(k)||new THREE.Vector3();list.forEach((n,i)=>{pos.set(n.id,c.clone().add(localPos(i,list.length,3+Math.sqrt(list.length)*1.8)));vel.set(n.id,new THREE.Vector3())})});
const ids=F.nodes.map(n=>n.id),tmp=new THREE.Vector3();
for(let it=0;it<280;it++){const cool=1-it/280;for(let a=0;a<N;a++){const pa=pos.get(ids[a]);for(let b=a+1;b<N;b++){const pb=pos.get(ids[b]);tmp.subVectors(pa,pb);let d2=tmp.lengthSq();if(d2>1000)continue;if(d2<.01){tmp.set((a%3)-1,(b%3)-1,1);d2=1}tmp.normalize().multiplyScalar(22/d2);vel.get(ids[a]).add(tmp);vel.get(ids[b]).sub(tmp)}}F.links.forEach(l=>{const pa=pos.get(l.from),pb=pos.get(l.to);tmp.subVectors(pb,pa);const d=tmp.length()||1;tmp.multiplyScalar((d-7.5)*.04/d);vel.get(l.from).add(tmp);vel.get(l.to).sub(tmp)});F.nodes.forEach(n=>{tmp.subVectors(centre.get(n.part)||new THREE.Vector3(),pos.get(n.id)).multiplyScalar(.05);vel.get(n.id).add(tmp)});F.nodes.forEach(n=>{const v=vel.get(n.id);v.clampLength(0,2.2*cool+.05);pos.get(n.id).add(v);v.multiplyScalar(.55)})}
// zones
const zones=[];groups.forEach((list,k)=>{const p=partById.get(k);if(!p)return;const c=new THREE.Vector3();list.forEach(n=>c.add(pos.get(n.id)));c.multiplyScalar(1/list.length);let r=0;list.forEach(n=>{r=Math.max(r,c.distanceTo(pos.get(n.id)))});r+=4;const color=partColor(k);const zone=new THREE.Mesh(new THREE.SphereGeometry(r,40,28),new THREE.MeshBasicMaterial({color,transparent:true,opacity:.07,depthWrite:false}));zone.position.copy(c);const el=document.createElement('div');el.className='logic-label';el.style.setProperty('--part-color',hex(color));el.innerHTML='<span class="emoji">'+esc(p.emoji)+'</span><b>'+esc(p.name)+'</b>';el.addEventListener('click',ev=>{ev.stopPropagation();showPart(p)});const label=new CSS2DObject(el);label.position.set(0,r+1,0);zone.add(label);zone.userData={part:p,el};logicGroup.add(zone);zones.push(zone)});
// nodes
const GEOM={start:new THREE.SphereGeometry(1.05,24,16),step:new THREE.BoxGeometry(2.2,1.2,1.2),decision:new THREE.OctahedronGeometry(1.45),store:new THREE.CylinderGeometry(1,1,1.5,24),end:new THREE.DodecahedronGeometry(1.1)};
const flowMeshes=[],meshById=new Map();F.nodes.forEach(n=>{const color=KIND_COLOR[n.kind]||partColor(n.part);const mesh=new THREE.Mesh(GEOM[n.kind]||GEOM.step,new THREE.MeshStandardMaterial({color,emissive:new THREE.Color(color),emissiveIntensity:.25,roughness:.45,transparent:true,opacity:.95}));mesh.position.copy(pos.get(n.id));const el=document.createElement('div');el.className='flow-label '+n.kind;el.style.setProperty('--c',hex(color));el.innerHTML='<span>'+KIND_ICON[n.kind]+'</span>'+esc(n.title);el.addEventListener('click',ev=>{ev.stopPropagation();showNode(n)});const label=new CSS2DObject(el);label.position.set(0,1.9,0);mesh.add(label);mesh.userData={node:n,el};logicGroup.add(mesh);flowMeshes.push(mesh);meshById.set(n.id,mesh)});
// arrows
const arrows=[];F.links.forEach(l=>{const a=pos.get(l.from),b=pos.get(l.to);const dir=b.clone().sub(a);const len=dir.length();if(len<.5)return;dir.normalize();const from=a.clone().add(dir.clone().multiplyScalar(1.3)),to=b.clone().sub(dir.clone().multiplyScalar(1.6));const mat=new THREE.LineBasicMaterial({color:0x64748b,transparent:true,opacity:.6});const line=new THREE.Line(new THREE.BufferGeometry().setFromPoints([from,to]),mat);const coneMat=new THREE.MeshBasicMaterial({color:0x64748b,transparent:true,opacity:.75});const cone=new THREE.Mesh(new THREE.ConeGeometry(.38,1,12),coneMat);cone.position.copy(to);cone.quaternion.setFromUnitVectors(new THREE.Vector3(0,1,0),dir);logicGroup.add(line,cone);let el=null,lab=null;if(l.label){el=document.createElement('div');el.className='flow-branch';el.textContent=l.label;lab=new CSS2DObject(el);lab.position.copy(from.clone().lerp(to,.5));logicGroup.add(lab)}arrows.push({l,mat,coneMat,el,line,cone,lab})});
// journey layout: selected journey is re-arranged top-down facing the camera
const home=new Map(ids.map(id=>[id,pos.get(id).clone()]));const target=new Map(home);let moving=false;const up0=new THREE.Vector3(0,1,0);
function updateArrows(){arrows.forEach(a=>{const pa=meshById.get(a.l.from).position,pb=meshById.get(a.l.to).position;const dir=pb.clone().sub(pa);if(dir.lengthSq()<.25)return;dir.normalize();const from=pa.clone().add(dir.clone().multiplyScalar(1.3)),to=pb.clone().sub(dir.clone().multiplyScalar(1.6));a.line.geometry.setFromPoints([from,to]);a.cone.position.copy(to);a.cone.quaternion.setFromUnitVectors(up0,dir);if(a.lab)a.lab.position.copy(from.lerp(to,.5))})}
function resetLayout(){ids.forEach(id=>target.set(id,home.get(id)));moving=true}
function layoutJourney(j){const set=new Set(j.nodes);const depth=new Map([[j.start,0]]);const queue=[j.start];while(queue.length){const id=queue.shift();(outLinks.get(id)||[]).forEach(l=>{if(set.has(l.to)&&!depth.has(l.to)){depth.set(l.to,depth.get(id)+1);queue.push(l.to)}})}let maxD=0;depth.forEach(d=>{maxD=Math.max(maxD,d)});j.nodes.forEach(id=>{if(!depth.has(id))depth.set(id,++maxD)});const levels=new Map();j.nodes.forEach(id=>{const d=depth.get(id);if(!levels.has(d))levels.set(d,[]);levels.get(d).push(id)});camera.updateMatrixWorld();const right=new THREE.Vector3().setFromMatrixColumn(camera.matrixWorld,0).normalize(),up=new THREE.Vector3().setFromMatrixColumn(camera.matrixWorld,1).normalize();const anchor=new THREE.Vector3();j.nodes.forEach(id=>anchor.add(home.get(id)));anchor.multiplyScalar(1/j.nodes.length);resetLayout();levels.forEach((list,d)=>list.forEach((id,i)=>target.set(id,anchor.clone().add(up.clone().multiplyScalar(-(d-maxD/2)*6)).add(right.clone().multiplyScalar((i-(list.length-1)/2)*Math.max(24,maxD*2.6))))))}
// state + highlight
let logicMode=false,active=null,hoverNode=null,logicDown=null;const techMeshes=[];
function focusSet(){if(hoverNode){const s=new Set([hoverNode.id]);(outLinks.get(hoverNode.id)||[]).forEach(l=>s.add(l.to));(inLinks.get(hoverNode.id)||[]).forEach(l=>s.add(l.from));return s}if(!active)return null;return active.ids}
function logicHighlight(){const s=focusSet();const main=hoverNode?.id||active?.main;flowMeshes.forEach(m=>{const n=m.userData.node;const on=!s||s.has(n.id);m.material.opacity=on?.95:.04;m.material.emissiveIntensity=main===n.id?.8:(on&&s?.45:.25);m.userData.el.dataset.on=on?'1':'0'});arrows.forEach(a=>{const on=!s||(s.has(a.l.from)&&s.has(a.l.to));const hot=Boolean(s)&&on;a.mat.opacity=on?(hot?.95:.35):.02;a.coneMat.opacity=on?(hot?1:.5):.02;a.mat.color.set(hot?0x67e8f9:0x64748b);a.coneMat.color.set(hot?0x67e8f9:0x64748b);if(a.el)a.el.dataset.on=on?'1':'0'});document.body.classList.toggle('journey-active',active?.kind==='journey');zones.forEach(z=>{z.material.opacity=active?.kind==='part'&&active.main===z.userData.part.id?.14:(active?.kind==='journey'?.02:.07)})}
// Overview: only zones and journey starts are labelled (others appear when the
// camera is close); with a journey/step selected only its labels show.
const labelTmp=new THREE.Vector3();function labelTick(){requestAnimationFrame(labelTick);if(!logicMode)return;if(moving){let still=true;flowMeshes.forEach(m=>{const t=target.get(m.userData.node.id);m.position.lerp(t,.14);if(m.position.distanceToSquared(t)>.0004)still=false});updateArrows();if(still)moving=false}const focused=Boolean(focusSet());const near=zoneR*.18,far=zoneR*.6;flowMeshes.forEach(m=>{const el=m.userData.el;const on=el.dataset.on!=='0';let o;if(focused)o=on?1:0;else if(m.userData.node.kind==='start')o=1;else{m.getWorldPosition(labelTmp);o=Math.max(0,Math.min(1,(far-camera.position.distanceTo(labelTmp))/(far-near)))}el.style.opacity=String(o);el.style.visibility=o>0.02?'visible':'hidden'});arrows.forEach(a=>{if(!a.el)return;const show=focused&&a.el.dataset.on!=='0';a.el.style.opacity=show?'1':'0';a.el.style.visibility=show?'visible':'hidden'})}labelTick();
function focusIds(list){const pts=list.map(id=>target.get(id)).filter(Boolean);if(!pts.length)return;const box=new THREE.Box3().setFromPoints(pts);const c=box.getCenter(new THREE.Vector3());const radius=Math.max(7,box.getSize(new THREE.Vector3()).length()/2+4);const dist=radius/Math.tan(THREE.MathUtils.degToRad(camera.fov)/2)*1.3;const dir=camera.position.clone().sub(controls.target);if(dir.lengthSq()<.001)dir.set(0,0,1);dir.normalize();if(!sidePanel.classList.contains('collapsed')){camera.updateMatrixWorld();const visibleWidth=2*dist*Math.tan(THREE.MathUtils.degToRad(camera.fov)/2)*camera.aspect;c.add(new THREE.Vector3().setFromMatrixColumn(camera.matrixWorld,0).normalize().multiplyScalar((sidePanel.offsetWidth+16)/innerWidth*visibleWidth/2))}focus={from:camera.position.clone(),to:c.clone().add(dir.multiplyScalar(dist)),targetFrom:controls.target.clone(),targetTo:c,start:performance.now(),duration:1000}}
const nodeLine=(id,extra)=>{const n=nodeById.get(id);return '<div class="step-row" data-node="'+esc(id)+'">'+(extra||'')+esc(KIND_ICON[n.kind]+' '+n.title)+'</div>'};
function wire(){sideContent.querySelectorAll('[data-node]').forEach(el=>el.addEventListener('click',()=>showNode(nodeById.get(el.dataset.node))));sideContent.querySelectorAll('[data-journey]').forEach(el=>el.addEventListener('click',()=>showJourney(F.journeys.find(j=>j.id===el.dataset.journey))));sideContent.querySelectorAll('[data-part]').forEach(el=>el.addEventListener('click',()=>showPart(partById.get(el.dataset.part))));const back=sideContent.querySelector('#flow-back');if(back)back.onclick=()=>{active=null;hoverNode=null;resetLayout();logicHighlight();logicIntro();focusIds(ids)};const jump=sideContent.querySelector('#logic-to-tech');if(jump)jump.onclick=()=>{const files=new Set(JSON.parse(jump.dataset.files));const techIds=DATA.nodes.filter(n=>files.has(n.metadata?.source_path)).map(n=>n.id);showView('tech');searchQuery=' ';searchMatches=new Set(techIds);applyHighlight();showSearchSummary(DATA.nodes.filter(n=>searchMatches.has(n.id)));focusOnIds(new Set(techIds))}}
function panel(html){sidePanel.classList.remove('empty');sideContent.innerHTML=html;wire()}
function logicIntro(){panel('<div class="kind">'+esc(T.title.toUpperCase())+'</div><h2>'+esc(T.title)+'</h2>'+(LOGIC.stale?'<div class="logic-stale">'+esc(T.stale)+'</div>':'')+'<div class="logic-summary">'+esc(T.intro)+'</div><div class="relations"><h3>'+esc(T.journeys)+'</h3>'+F.journeys.map(j=>'<button type="button" class="journey-btn" data-journey="'+esc(j.id)+'">▶ '+esc(j.title)+'<small>'+esc(j.summary)+' · '+j.nodes.length+' '+esc(T.steps)+'</small></button>').join('')+'</div><div class="relations"><h3>'+esc(T.parts)+'</h3>'+LOGIC.parts.map(p=>'<div class="step-row" data-part="'+esc(p.id)+'">'+esc(p.emoji+' '+p.name)+' <small>— '+esc(p.summary)+'</small></div>').join('')+'</div>')}
function showJourney(j){active={kind:'journey',main:j.start,ids:new Set(j.nodes)};hoverNode=null;layoutJourney(j);logicHighlight();let html='<button type="button" id="flow-back">'+esc(T.back)+'</button><div class="kind">'+esc(T.journeys.toUpperCase())+'</div><h2>▶ '+esc(j.title)+'</h2><div class="logic-summary">'+esc(j.summary)+'</div><div class="relations">';j.nodes.forEach((id,i)=>{const n=nodeById.get(id);const outs=(outLinks.get(id)||[]).filter(l=>active.ids.has(l.to));const branches=n.kind==='decision'?outs.map(l=>'<span class="branch">'+esc((l.label||'…')+' → '+nodeById.get(l.to).title)+'</span>').join(''):'';html+='<div class="step-row" data-node="'+esc(id)+'"><span class="step-num">'+(i+1)+'.</span>'+esc(KIND_ICON[n.kind]+' '+n.title)+branches+'</div>'});panel(html+'</div>');focusIds(j.nodes)}
function showNode(n){hoverNode=null;const near=[n.id,...(outLinks.get(n.id)||[]).map(l=>l.to),...(inLinks.get(n.id)||[]).map(l=>l.from)];active={kind:'node',main:n.id,ids:new Set(near)};logicHighlight();const p=partById.get(n.part);const ins=(inLinks.get(n.id)||[]).map(l=>nodeLine(l.from,l.label?'<small>['+esc(l.label)+']</small> ':''));const outs=(outLinks.get(n.id)||[]).map(l=>nodeLine(l.to,l.label?'<small>['+esc(l.label)+']</small> ':''));const js=F.journeys.filter(j=>j.nodes.includes(n.id));panel('<button type="button" id="flow-back">'+esc(T.back)+'</button><div class="kind">'+esc(T.kinds[n.kind]||n.kind)+'</div><h2>'+esc(KIND_ICON[n.kind]+' '+n.title)+'</h2><div class="logic-summary">'+esc(n.detail)+'</div>'+(p?'<div class="step-row" data-part="'+esc(p.id)+'">'+esc(T.part)+': '+esc(p.emoji+' '+p.name)+'</div>':'')+(ins.length?'<div class="relations"><h3>'+esc(T.before)+'</h3>'+ins.join('')+'</div>':'')+(outs.length?'<div class="relations"><h3>'+esc(T.after)+'</h3>'+outs.join('')+'</div>':'')+(js.length?'<div class="relations"><h3>'+esc(T.inJourneys)+'</h3>'+js.map(j=>'<button type="button" class="journey-btn" data-journey="'+esc(j.id)+'">▶ '+esc(j.title)+'</button>').join('')+'</div>':''));focusIds(near)}
function showPart(p){const own=F.nodes.filter(n=>n.part===p.id).map(n=>n.id);active={kind:'part',main:p.id,ids:new Set(own)};hoverNode=null;resetLayout();logicHighlight();panel('<button type="button" id="flow-back">'+esc(T.back)+'</button><div class="kind">'+esc(T.part.toUpperCase())+'</div><h2>'+esc(p.emoji+' '+p.name)+'</h2><div class="logic-summary">'+esc(p.summary)+'</div><div class="relations">'+own.map(id=>nodeLine(id)).join('')+'</div><details class="relations"><summary>'+p.files.length+' source file(s)</summary>'+p.files.map(f=>'<div><small>'+esc(f)+'</small></div>').join('')+'</details><p><button type="button" id="logic-to-tech" data-files="'+esc(JSON.stringify(p.files))+'">'+esc(T.seeTech)+'</button></p>');focusIds(own.length?own:ids)}
function setMode(on){logicMode=on;logicGroup.visible=on;hovered=null;selected=null;active=null;hoverNode=null;if(!on)resetLayout();searchQuery='';searchMatches=new Set();applyHighlight();logicHighlight();if(on)statsEl.textContent=T.stats(F.journeys.length,F.nodes.length);if(on){logicIntro();focusIds(ids)}}
VIEWS.logic={enter:()=>setMode(true),leave:()=>setMode(false)};
document.querySelector('#reset').addEventListener('click',()=>{if(!logicMode)return;active=null;hoverNode=null;resetLayout();logicHighlight();logicIntro();focusIds(ids)});
renderer.domElement.addEventListener('pointerdown',ev=>{logicDown={x:ev.clientX,y:ev.clientY}});
const pick=ev=>{pointer.x=(ev.clientX/innerWidth)*2-1;pointer.y=-(ev.clientY/innerHeight)*2+1;ray.setFromCamera(pointer,camera);return ray.intersectObjects(flowMeshes)[0]};
renderer.domElement.addEventListener('pointermove',ev=>{if(!logicMode)return;const hit=pick(ev);const n=hit?.object.userData.node||null;if(n?.id!==hoverNode?.id){hoverNode=n;logicHighlight()}renderer.domElement.style.cursor=hit?'pointer':'grab'});
renderer.domElement.addEventListener('click',ev=>{if(!logicMode)return;const dragged=logicDown&&((ev.clientX-logicDown.x)**2+(ev.clientY-logicDown.y)**2)>36;logicDown=null;if(dragged)return;const hit=pick(ev);if(hit)showNode(hit.object.userData.node);else if(active){active=null;hoverNode=null;resetLayout();logicHighlight();logicIntro();focusIds(ids)}});
}

if(DOCS){
// Documentation: documents grouped in topic zones, arrows are typed relations.
const docGroup=new THREE.Group();docGroup.visible=false;scene.add(docGroup);let docMode=false,docActive=null,docHover=null,docDown=null;
const DPAL=[0x67e8f9,0xa78bfa,0x34d399,0xf472b6,0x60a5fa,0xfdba74,0xa3e635,0xfb7185,0xfbbf24];
const REL_COLOR={references:0x94a3b8,details:0x60a5fa,applies:0x34d399,depends_on:0xfbbf24,updates:0xa78bfa,contradicts:0xfb7185,same_subject:0x64748b};
const REL_LABEL={references:'references',details:'details',applies:'applies',depends_on:'depends on',updates:'updates',contradicts:'contradicts',same_subject:'same subject as'};
const hexd=c=>'#'+c.toString(16).padStart(6,'0');
const topicIdx=new Map(DOCS.topics.map((t,i)=>[t.id,i])),topicById=new Map(DOCS.topics.map(t=>[t.id,t])),docById=new Map(DOCS.docs.map(d=>[d.id,d]));
const tColor=id=>DPAL[(topicIdx.get(id)??0)%DPAL.length];
const dOut=new Map(),dIn=new Map();DOCS.relations.forEach(r=>{if(!dOut.has(r.from))dOut.set(r.from,[]);dOut.get(r.from).push(r);if(!dIn.has(r.to))dIn.set(r.to,[]);dIn.get(r.to).push(r)});
const degree=id=>(dOut.get(id)||[]).length+(dIn.get(id)||[]).length;
// deterministic force layout
const DN=DOCS.docs.length,dR=Math.max(48,Math.sqrt(DN)*10),dCentre=new Map(DOCS.topics.map((t,i)=>[t.id,localPos(i,DOCS.topics.length,dR)])),dPos=new Map(),dVel=new Map(),dGroups=new Map();
DOCS.docs.forEach(d=>{if(!dGroups.has(d.topic))dGroups.set(d.topic,[]);dGroups.get(d.topic).push(d)});
dGroups.forEach((list,k)=>{const c=dCentre.get(k)||new THREE.Vector3();list.forEach((d,i)=>{dPos.set(d.id,c.clone().add(localPos(i,list.length,3+Math.sqrt(list.length)*1.9)));dVel.set(d.id,new THREE.Vector3())})});
const dIds=DOCS.docs.map(d=>d.id),dt=new THREE.Vector3();
for(let it=0;it<260;it++){const cool=1-it/260;for(let a=0;a<DN;a++){const pa=dPos.get(dIds[a]);for(let b=a+1;b<DN;b++){const pb=dPos.get(dIds[b]);dt.subVectors(pa,pb);let d2=dt.lengthSq();if(d2>900)continue;if(d2<.01){dt.set((a%3)-1,(b%3)-1,1);d2=1}dt.normalize().multiplyScalar(20/d2);dVel.get(dIds[a]).add(dt);dVel.get(dIds[b]).sub(dt)}}DOCS.relations.forEach(r=>{const pa=dPos.get(r.from),pb=dPos.get(r.to);if(!pa||!pb)return;dt.subVectors(pb,pa);const d=dt.length()||1;dt.multiplyScalar((d-8)*.03/d);dVel.get(r.from).add(dt);dVel.get(r.to).sub(dt)});DOCS.docs.forEach(d=>{dt.subVectors(dCentre.get(d.topic)||new THREE.Vector3(),dPos.get(d.id)).multiplyScalar(.05);dVel.get(d.id).add(dt)});DOCS.docs.forEach(d=>{const v=dVel.get(d.id);v.clampLength(0,2*cool+.05);dPos.get(d.id).add(v);v.multiplyScalar(.55)})}
// topic zones
const dZones=[];dGroups.forEach((list,k)=>{const t=topicById.get(k);if(!t)return;const c=new THREE.Vector3();list.forEach(d=>c.add(dPos.get(d.id)));c.multiplyScalar(1/list.length);let r=0;list.forEach(d=>{r=Math.max(r,c.distanceTo(dPos.get(d.id)))});r+=4;const color=tColor(k);const zone=new THREE.Mesh(new THREE.SphereGeometry(r,40,28),new THREE.MeshBasicMaterial({color,transparent:true,opacity:.07,depthWrite:false}));zone.position.copy(c);const el=document.createElement('div');el.className='topic-label';el.style.setProperty('--c',hexd(color));el.innerHTML='<span>'+esc(t.emoji)+'</span><b>'+esc(t.name)+'</b> <small>'+list.length+'</small>';el.addEventListener('click',ev=>{ev.stopPropagation();showTopic(t)});const lab=new CSS2DObject(el);lab.position.set(0,r+1,0);zone.add(lab);zone.userData={topic:t,el};docGroup.add(zone);dZones.push(zone)});
// documents
const maxDeg=Math.max(1,...dIds.map(degree)),docMeshes=[],docMeshById=new Map();
DOCS.docs.forEach(d=>{const color=tColor(d.topic);const size=.7+1.1*Math.sqrt(degree(d.id)/maxDeg);const mesh=new THREE.Mesh(new THREE.BoxGeometry(size*1.4,size*1.8,size*.35),new THREE.MeshStandardMaterial({color,emissive:new THREE.Color(color),emissiveIntensity:.25,roughness:.45,transparent:true,opacity:.95}));mesh.position.copy(dPos.get(d.id));const el=document.createElement('div');el.className='doc-label';el.style.setProperty('--c',hexd(color));el.textContent=d.title;el.addEventListener('click',ev=>{ev.stopPropagation();showDoc(d)});const lab=new CSS2DObject(el);lab.position.set(0,size*1.3+.6,0);mesh.add(lab);mesh.userData={doc:d,el};docGroup.add(mesh);docMeshes.push(mesh);docMeshById.set(d.id,mesh)});
// relations
const dArrows=[];DOCS.relations.forEach(r=>{const a=dPos.get(r.from),b=dPos.get(r.to);if(!a||!b)return;const dir=b.clone().sub(a);if(dir.lengthSq()<.25)return;dir.normalize();const from=a.clone().add(dir.clone().multiplyScalar(1.4)),to=b.clone().sub(dir.clone().multiplyScalar(1.7));const color=REL_COLOR[r.type]||0x94a3b8;const mat=new THREE.LineBasicMaterial({color,transparent:true,opacity:.55});const line=new THREE.Line(new THREE.BufferGeometry().setFromPoints([from,to]),mat);const cmat=new THREE.MeshBasicMaterial({color,transparent:true,opacity:.7});const cone=new THREE.Mesh(new THREE.ConeGeometry(.35,1,12),cmat);cone.position.copy(to);cone.quaternion.setFromUnitVectors(new THREE.Vector3(0,1,0),dir);const el=document.createElement('div');el.className='doc-rel';el.style.setProperty('--c',hexd(color));el.textContent=REL_LABEL[r.type]||r.type;const lab=new CSS2DObject(el);lab.position.copy(from.clone().lerp(to,.5));docGroup.add(line,cone,lab);dArrows.push({r,mat,cmat,el})});
function docFocusSet(){const act=docHover?{kind:'doc',ids:new Set([docHover.id,...(dOut.get(docHover.id)||[]).map(r=>r.to),...(dIn.get(docHover.id)||[]).map(r=>r.from)])}:docActive;return act?act.ids:null}
function docHighlight(){const s=docFocusSet();const main=docHover?.id||docActive?.main;docMeshes.forEach(m=>{const on=!s||s.has(m.userData.doc.id);m.material.opacity=on?.95:.05;m.material.emissiveIntensity=main===m.userData.doc.id?.8:(on&&s?.45:.25);m.userData.el.dataset.on=on?'1':'0'});dArrows.forEach(a=>{const on=!s||(s.has(a.r.from)&&s.has(a.r.to));a.mat.opacity=on?(s?.95:.45):.03;a.cmat.opacity=on?(s?1:.6):.03;a.el.dataset.on=on&&s?'1':'0'});const liveTopics=s?new Set([...s].map(id=>docById.get(id)?.topic)):null;dZones.forEach(z=>{z.material.opacity=docActive?.kind==='topic'&&docActive.main===z.userData.topic.id?.14:.07;z.userData.el.style.opacity=!liveTopics||liveTopics.has(z.userData.topic.id)?'1':'.18'})}
const dTmp=new THREE.Vector3(),topDocs=new Set(DN<=12?dIds:[]);
function docLabelTick(){requestAnimationFrame(docLabelTick);if(!docMode)return;const s=docFocusSet();const near=dR*.35,far=dR*1.1;docMeshes.forEach(m=>{const el=m.userData.el;let o;if(s)o=el.dataset.on==='1'?1:0;else if(topDocs.has(m.userData.doc.id))o=.95;else{m.getWorldPosition(dTmp);o=Math.max(0,Math.min(1,(far-camera.position.distanceTo(dTmp))/(far-near)))}el.style.opacity=String(o);el.style.visibility=o>.02?'visible':'hidden'});dArrows.forEach(a=>{const show=a.el.dataset.on==='1';a.el.style.opacity=show?'1':'0';a.el.style.visibility=show?'visible':'hidden'})}docLabelTick();
function focusDocs(list){const pts=list.map(id=>dPos.get(id)).filter(Boolean);if(!pts.length)return;const box=new THREE.Box3().setFromPoints(pts);const c=box.getCenter(new THREE.Vector3());const radius=Math.max(7,box.getSize(new THREE.Vector3()).length()/2+4);const dist=radius/Math.tan(THREE.MathUtils.degToRad(camera.fov)/2)*1.3;const dir=camera.position.clone().sub(controls.target);if(dir.lengthSq()<.001)dir.set(0,0,1);dir.normalize();if(!sidePanel.classList.contains('collapsed')){camera.updateMatrixWorld();const w=2*dist*Math.tan(THREE.MathUtils.degToRad(camera.fov)/2)*camera.aspect;c.add(new THREE.Vector3().setFromMatrixColumn(camera.matrixWorld,0).normalize().multiplyScalar((sidePanel.offsetWidth+16)/innerWidth*w/2))}focus={from:camera.position.clone(),to:c.clone().add(dir.multiplyScalar(dist)),targetFrom:controls.target.clone(),targetTo:c,start:performance.now(),duration:1000}}
const docRow=(id,prefix,suffix)=>{const d=docById.get(id);return '<div class="step-row" data-doc="'+esc(id)+'">'+(prefix||'')+esc(d.title)+'<span class="doc-type">'+esc(d.type)+'</span>'+(suffix||'')+'</div>'};
function dPanel(html){sidePanel.classList.remove('empty');sideContent.innerHTML=html;sideContent.querySelectorAll('[data-doc]').forEach(el=>el.addEventListener('click',()=>showDoc(docById.get(el.dataset.doc))));sideContent.querySelectorAll('[data-topic]').forEach(el=>el.addEventListener('click',()=>showTopic(topicById.get(el.dataset.topic))));const back=sideContent.querySelector('#doc-back');if(back)back.onclick=()=>{docActive=null;docHover=null;docHighlight();docIntro();focusDocs(dIds)}}
function docIntro(){const counts={};DOCS.relations.forEach(r=>{counts[r.type]=(counts[r.type]||0)+1});dPanel('<div class="kind">DOCUMENTATION</div><h2>'+DOCS.docs.length+' documents in '+DOCS.topics.length+' topics</h2><div class="logic-summary">Each coloured zone is a topic and each card is a document. Arrows show how one document relates to another; click a document or a topic to explore.</div><div class="relations"><h3>Topics</h3>'+DOCS.topics.map(t=>'<div class="step-row" data-topic="'+esc(t.id)+'">'+esc(t.emoji+' '+t.name)+' <small>· '+(dGroups.get(t.id)||[]).length+(t.summary?' — '+esc(t.summary):'')+'</small></div>').join('')+'</div><div class="relations"><h3>Relations</h3>'+Object.keys(REL_LABEL).filter(k=>counts[k]).map(k=>'<div class="rel-key"><i style="--c:'+hexd(REL_COLOR[k])+'"></i>'+esc(REL_LABEL[k])+' <small>'+counts[k]+'</small></div>').join('')+'</div>')}
function showDoc(d){docHover=null;const ids=new Set([d.id,...(dOut.get(d.id)||[]).map(r=>r.to),...(dIn.get(d.id)||[]).map(r=>r.from)]);docActive={kind:'doc',main:d.id,ids};docHighlight();const t=topicById.get(d.topic);const outs=(dOut.get(d.id)||[]).map(r=>docRow(r.to,'→ <em>'+esc(REL_LABEL[r.type]||r.type)+'</em> '));const ins=(dIn.get(d.id)||[]).map(r=>docRow(r.from,'',' → <em>'+esc(REL_LABEL[r.type]||r.type)+'</em> this'));dPanel('<button type="button" id="doc-back">← All topics</button><div class="kind">'+esc(d.type)+'</div><h2>'+esc(d.title)+'</h2><div class="logic-summary">'+esc(d.summary)+'</div>'+(t?'<div class="step-row" data-topic="'+esc(t.id)+'">Topic: '+esc(t.emoji+' '+t.name)+'</div>':'')+'<div class="step-row"><a href="../'+esc(encodeURI(d.path))+'" target="_blank" rel="noopener" style="color:var(--accent)">'+esc(d.path)+'</a></div>'+(outs.length?'<div class="relations"><h3>This document</h3>'+outs.join('')+'</div>':'')+(ins.length?'<div class="relations"><h3>Other documents</h3>'+ins.join('')+'</div>':''));focusDocs([...ids])}
function showTopic(t){const own=(dGroups.get(t.id)||[]).map(d=>d.id);docActive={kind:'topic',main:t.id,ids:new Set(own)};docHover=null;docHighlight();dPanel('<button type="button" id="doc-back">← All topics</button><div class="kind">TOPIC</div><h2>'+esc(t.emoji+' '+t.name)+'</h2><div class="logic-summary">'+esc(t.summary||'')+'</div><div class="relations">'+own.map(id=>docRow(id)).join('')+'</div>');focusDocs(own.length?own:dIds)}
VIEWS.docs={enter(){docMode=true;docGroup.visible=true;docActive=null;docHover=null;docHighlight();statsEl.textContent=DOCS.docs.length+' documents · '+DOCS.topics.length+' topics · '+DOCS.relations.length+' relations';docIntro();focusDocs(dIds)},leave(){docMode=false;docGroup.visible=false;docActive=null;docHover=null}};
document.querySelector('#reset').addEventListener('click',()=>{if(!docMode)return;docActive=null;docHover=null;docHighlight();docIntro();focusDocs(dIds)});
renderer.domElement.addEventListener('pointerdown',ev=>{docDown={x:ev.clientX,y:ev.clientY}});
const dPick=ev=>{pointer.x=(ev.clientX/innerWidth)*2-1;pointer.y=-(ev.clientY/innerHeight)*2+1;ray.setFromCamera(pointer,camera);return ray.intersectObjects(docMeshes)[0]};
renderer.domElement.addEventListener('pointermove',ev=>{if(!docMode)return;const hit=dPick(ev);const d=hit?.object.userData.doc||null;if(d?.id!==docHover?.id){docHover=d;docHighlight()}renderer.domElement.style.cursor=hit?'pointer':'grab'});
renderer.domElement.addEventListener('click',ev=>{if(!docMode)return;const dragged=docDown&&((ev.clientX-docDown.x)**2+(ev.clientY-docDown.y)**2)>36;docDown=null;if(dragged)return;const hit=dPick(ev);if(hit)showDoc(hit.object.userData.doc);else if(docActive){docActive=null;docHover=null;docHighlight();docIntro();focusDocs(dIds)}});
}
document.querySelectorAll('#views [data-view]').forEach(b=>b.addEventListener('click',()=>showView(b.dataset.view)));
{let savedView=null;try{savedView=localStorage.getItem('cm-graph-view')}catch{}showView(VIEWS[savedView]?savedView:(VIEWS.logic?'logic':VIEWS.docs?'docs':'tech'))}
</script></body></html>`;
