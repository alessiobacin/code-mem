...
function scanASTDeep(cwd, noAst) {
  ...
  function walkASTWithAcorn(acorn, acornLoose, content, fileBase, filePath) {
    ...
    // Local bindings: imported name -> repo-relative target file. Resolves
    // BOTH relative (`./auth`) and later lets the linking pass emit EXTRACTED
    // `imports` + `calls` edges that graphify has and cm used to miss.
    const localBindings = new Map(); // name -> targetRel
    const resolveLocal = (request) => {
      if (!request || !request.startsWith(".")) return "";
      const base = normalize(join(dirname(relPath), request)).replace(/\\/g, "/");
      const cands = [base, `${base}.js`, `${base}.jsx`, `${base}.mjs`, `${base}.cjs`, `${base}.ts`, `${base}.tsx`, `${base}/index.js`, `${base}/index.ts`];
      const found = cands.find((c) => fileSymbols.has(c) || existsSync(join(cwd, c)));
      return found || "";
    };
      ...
      if (req.startsWith(".")) {
        ...
        if (destr) {
          for (const nm of destr[1].split(",").map((s) => s.trim().split(/\s+as\s+/).pop().trim()).filter(Boolean)) {
            if (/^[A-Za-z_$][\w$]*$/.test(nm)) pending.push({ name: nm, request: req });
          }
        } else {
          const idm = /(?:const|let|var)\s+(\w+)\s*=\s*require/.exec(before) || /import\s+(\w+)\s+from/.exec(before);
          if (idm) pending.push({ name: idm[1], request: req });
        }
      } else {
  ...
  function walkGenericFile(content, fileBase, filePath, lang) {
    ...
    while ((match = patterns.imp.exec(content)) !== null) {
      ...
      if (lang === "py" && !/^[./]/.test(req) && /^[A-Za-z_]\w*(?:\.[A-Za-z_]\w*)*$/.test(req)) {
        ...
        for (const nm of names) if (/^[A-Za-z_]\w*$/.test(nm)) pending.push({ name: nm, request: `./${req.split(".")[0]}` });
      }
      if (/^[./]/.test(req)) {
        ...
        for (const nm of names) pending.push({ name: nm, request: req });
      } else {
  ...
  const resolveLocalLink = (fromRel, request) => {
    ...
    return cands.find((c) => fileSymbols.has(c)) || "";
  };
  ...
  for (const rec of fileCallData) {
    ...
    for (const target of boundTargets) {
      const tgt = fileSymbols.get(target);
    ...
    for (const call of rec.calls || []) {
      ...
      if (call.caller) {
        ...
        else addEdge(callerId, fileSymbols.get(target)?.fileId || rec.fileId, "calls", "INFERRED");
      } else if (calleeId) {
